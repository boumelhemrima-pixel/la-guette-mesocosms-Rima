...
#           CONFIG
# =============================
setwd("C:/Users/rboumelh/Desktop/Mesocosms/Modeling GPP/GPPmax_by campaign/by replicate/GPP_by treatment")

data_csv   <- "C:/Users/rboumelh/Desktop/Mesocosms/Modeling Reco/Full data_hourly averages.csv"
param_xlsx <- file.path(getwd(), "GPPmax_Model_Results_All_withReplicates.xlsx")  # a,b,Topt per B1..G3
...
# Replicate-specific rectangular-hyperbola K
k_map <- c(
  B1=407.0161559, B2=396.1684879, B3=744.6314954,
  BG1=1452.782738, BG2=426.3371158, BG3=516.7650617,
  BGS1=342.1892956, BGS2=556.3127428, BGS3=483.8460726,
...
gpp_Model <- function(a, b, Topt, V2, Ta, PAR, k) {
...
need_cols <- c("SubsetType","Model","a","b","Topt")
...
params_df <- params_df %>%
  ...
  mutate(across(c(a,b,Topt), ~ suppressWarnings(as.numeric(.)))) %>%
  filter(Model == "Model_LAI")   # <<< LAI-only (remove VI)

# Nested params: params_nested[["Model_LAI"]][["B1"]] -> list(a=.., b=.., Topt=..)
params_by_model <- split(params_df, params_df$Model)
params_nested <- lapply(params_by_model, function(df) {
  setNames(lapply(seq_len(nrow(df)), function(i) as.list(df[i, c("a","b","Topt")])),
           df$SubsetType)
})
...
run_model <- function(model_name) {
  ...
  for (id in ids) {
    ...
    if (!is.finite(k_val)) {
      warning(sprintf("[%s] missing K for %s → skipping", model_name, id))
      next
    }
    ...
    a  <- par_id$a; b <- par_id$b; Topt <- par_id$Topt
